...
contract('Freedom', function(accounts)
{
    ...
    it('initializes the contract with correct values', function()
    {
        return Freedom.deployed().then(function(instance){
            ...
            assert.equal(name, 'Freedom', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol) {
            assert.equal(symbol, 'FRE', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard) {
        	assert.equal(standard, 'Freedom v1.0', 'has the correct standard');
        });
    })
